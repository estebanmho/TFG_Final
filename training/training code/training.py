...
X = np.loadtxt('./data/comprobar_simb.csv', delimiter=",", dtype='float64', usecols=range(0,6), skiprows=1)
L = np.loadtxt('./data/comprobar_simb.csv', delimiter=",", dtype='int', usecols=[6], skiprows=1)
...
print('training...')
h = model.fit(X, d, epochs=1500, verbose=0, callbacks=[PeriodicPrint(100)])
print('trained')

model.save('comprobar_simb.h5')
